/* ─────────────────────────────────────────────────────────────────────────────
   RETIRED — the payment reconciliation sweep.
   Commented out 21 Sep 2026, kept rather than deleted so the behaviour it
   encoded stays readable while the Django path is still new.

   Replaced by: apps/wallet/management/commands/reconcile_payments.py
   ...
   is what stops it. Nothing calls it either way — the client makes no
   PostgREST or functions.invoke() call at all (HANDOFF §18).
